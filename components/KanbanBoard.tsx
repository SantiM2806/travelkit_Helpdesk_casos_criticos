...
export default function KanbanBoard({ allTickets, onMoveRequest }: KanbanBoardProps) {
  ...
  const byEstado: Record<string, Ticket[]> = { 'Abierto': [], 'En proceso': [], 'Resuelto': [], 'Otra área': [] };
  ...
  function onPointerDown(e: React.PointerEvent<HTMLDivElement>) {
    ...
    dragStateRef.current = {
      card: card as HTMLElement, ticketId: card.dataset.ticketId ?? '',
      startX: e.clientX, startY: e.clientY,
      offsetX: e.clientX - rect.left, offsetY: e.clientY - rect.top, width: rect.width,
      activated: false, floater: null, activeCol: null, lastVX: 0, lastX: e.clientX,
    };
  ...
  function onPointerMove(e: PointerEvent) {
    ...
    if (!ds.activated) { if (Math.hypot(dx, dy) < 6) return; ds.activated = true; activateDrag(e); }
  ...
  function highlightCol(x: number, y: number) {
    ...
    if (found) found.classList.add('kanban-col-drag-over');
    ds.activeCol = found;
  }
  ...
  function onPointerUp() {
    ...
    ds.card.style.opacity = ''; ds.card.style.transform = ''; ds.card.style.borderStyle = ''; ds.card.style.borderColor = ''; ds.card.style.pointerEvents = '';
    ...
    if (ds.activated && ds.activeCol) {
      ...
      if (ticket && ticket.estado !== targetEstado) onMoveRequest(ds.ticketId, ticket.estado, targetEstado);
    }
  ...
  return (
    <div className="grid grid-cols-4 gap-4 items-start max-[1200px]:grid-cols-2 max-[900px]:grid-cols-1">
      {COLS.map(col => {
        ...
        return (
          <div key={col.estado} data-estado={col.estado} className="kanban-col-el bg-tk-bg2 border border-tk-border rounded-lg min-h-[480px] flex flex-col relative transition-[border-color,background] duration-[0.15s] max-[900px]:min-h-0">
            <div className="kanban-col-header-inner px-4 py-3 border-b border-tk-border flex items-center gap-2 flex-shrink-0">
              ...
              <div className={`font-mono text-[11px] font-semibold tracking-[0.1em] uppercase ${col.colorCls}`}>{col.estado}</div>
              <div className="font-mono text-[10px] px-[7px] py-[2px] rounded-full bg-tk-bg3 text-tk-text3 border border-tk-border ml-auto">{tickets.length}</div>
            </div>
            <div className="kanban-col-body-inner p-3 flex flex-col gap-2 flex-1 min-h-[200px]">
              {tickets.length === 0 ? (
                <div className="flex items-center justify-center h-20 border border-dashed border-tk-border rounded-md font-mono text-[10px] tracking-[0.06em] uppercase text-tk-text3">Sin tickets</div>
              ) : (
                tickets.map(t => (
                  <div key={t.ticket_id} data-ticket-id={t.ticket_id} onPointerDown={onPointerDown} className="kanban-card bg-tk-bg3 border border-tk-border rounded-md px-[14px] py-3 cursor-grab select-none transition-[border-color,transform,box-shadow,opacity] duration-[0.12s,0.15s,0.15s,0.2s] hover:border-tk-border2 hover:shadow-[0_3px_12px_rgba(0,0,0,0.25)] hover:-translate-y-px active:cursor-grabbing">
                    <div className="flex items-start justify-between gap-2 mb-[7px]">
                      <span className="font-mono text-[11px] font-semibold text-tk-text2 tracking-[0.04em]">{t.ticket_id}</span>
                      <span className={badgePrioridad(t.prioridad).cls}>{badgePrioridad(t.prioridad).label}</span>
                    </div>
                    <div className="text-xs text-tk-text2 mb-[5px] whitespace-nowrap overflow-hidden text-ellipsis">{t.email}</div>
                    <div className="card-desc text-xs text-tk-text leading-[1.5] mb-2.5">{t.descripcion}</div>
                    <div className="flex items-center gap-1.5 flex-wrap">
                      {badgeCat(t.categoria) && <span className={badgeCat(t.categoria)!.cls}>{badgeCat(t.categoria)!.label}</span>}
                    </div>
                    {t.area && (
