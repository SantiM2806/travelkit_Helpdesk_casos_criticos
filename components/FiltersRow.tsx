  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center flex-wrap gap-2 mb-5">
      <div className="flex items-center flex-wrap gap-2">
        ...
        {ESTADOS.map(e => (
          <button key={e} onClick={() => onEstadoChange(e)} className={`${BASE_CHIP} ${activeEstado === e ? ESTADO_ACTIVE[e] : ''}`}>
            {e}
          </button>
        ))}
        ...
        {PRIORIDADES.map(p => (
          <button key={p} onClick={() => onPrioridadChange(p)} className={`${BASE_CHIP} ${activePrioridad === p ? PRIORIDAD_ACTIVE[p] : ''}`}>
            {p}
          </button>
        ))}
      </div>
      ...
      <div className="sm:ml-auto relative flex items-center w-full sm:w-auto">
        <svg className="absolute left-2.5 w-[13px] h-[13px] text-tk-text3 pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
