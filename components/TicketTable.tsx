...
export default function TicketTable({ filteredTickets, allTickets, isLoading }: TicketTableProps) {
  ...
  useEffect(() => {
    ...
    rows.forEach(row => requestAnimationFrame(() => row.classList.add('row-visible')));
  }, [filteredTickets]);
      ...
      <table className="w-full border-collapse text-[13px]">
        <thead>
          <tr>
            {['Ticket ID', 'Email', 'Categoría', 'Prioridad', 'Descripción', 'Estado', 'Fecha'].map((h, i) => (
              <th key={h} className={`font-mono text-[10px] font-semibold tracking-[0.12em] uppercase text-tk-text3 py-3 px-4 text-left bg-tk-bg2 border-b border-tk-border whitespace-nowrap select-none first:pl-5 last:pr-5 ${[2, 4, 6].includes(i) ? 'hidden md:table-cell' : ''}`}>
                {h}
              </th>
            ))}
          </tr>
        ...
        <tbody ref={tbodyRef}>
          ...
          {showRows && filteredTickets.map((t, i) => {
            ...
            return (
              <tr key={t.ticket_id} className="ticket-row border-b border-tk-border last:border-b-0 hover:bg-tk-bg3" style={{ transitionDelay: `${i * 30}ms` }}>
                <td className="py-3 px-4 pl-5 font-mono text-xs text-tk-text2 whitespace-nowrap">
                  <span className="inline-flex items-center gap-1.5">
                    ...
                    {t.imagen_url && (
                      <a href={t.imagen_url} target="_blank" rel="noopener noreferrer" className="text-tk-text3 hover:text-tk-accent2 transition-colors duration-[0.12s]">
                ...
                <td className="py-3 px-4 hidden md:table-cell">
                  {catBadge ? <span className={catBadge.cls}>{catBadge.label}</span> : <span className="text-tk-text3">—</span>}
                </td>
