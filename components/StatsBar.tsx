    ...
    <div className="bg-tk-bg2 border-b border-tk-border grid grid-cols-2 md:grid-cols-4">
      {CELLS.map((cell, i) => (
        ...
        >
          ...
          <div className={`font-mono text-[28px] font-semibold leading-[1.1] mb-[14px] ${cell.colorClass}`}>
            {values[i] || '—'}
          </div>
          <div className="h-[3px] bg-tk-border absolute bottom-0 left-0 right-0">
            <div ref={el => { barsRef.current[i] = el; }} className={`stat-bar-fill ${cell.barColor}`} />
          </div>
        </div>
      ))}
    </div>
