...
async function createTables() {
  ...
  console.log('\nVerifying tables...');
  try {
    const { data, error } = await supabase.from('tickets').select('ticket_id').limit(1);
    if (!error) console.log('✓ Table tickets verified');
  } catch (e: any) {
    console.log('tickets:', e.message);
  }
  try {
    const { data, error } = await supabase.from('categorias_it').select('id').limit(1);
    if (!error) console.log('✓ Table categorias_it verified');
  } catch (e: any) {
    console.log('categorias_it:', e.message);
  }
  try {
    const { data, error } = await supabase.from('solicitudes_it').select('id').limit(1);
