...
export default function ClientePanel({ clienteId, onClose, onUpdated }: Props) {
  ...
  const inputCls = "w-full px-3 py-2 rounded-lg bg-tk-bg3 border border-tk-border2 text-tk-text text-[13px] focus:outline-none focus:border-tk-accent2 transition-colors font-sans";
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: c }, { data: a }] = await Promise.all([
      ...
      sb.from('cliente_actividades').select('*').eq('cliente_id', clienteId).order('created_at', { ascending: false }),
    ]);
    if (!c) { onClose(); return; }
  ...
  async function handleSaveEdit() {
    ...
    setSaving(false);
    if (!error) { setEditing(false); load(); onUpdated(); }
  }
  ...
  return (
    <div className="fixed inset-0 z-[150] flex justify-end">
      ...
      {/* Panel */}
      <div className="relative w-full max-w-[860px] bg-tk-bg h-full flex flex-col shadow-2xl border-l border-tk-border animate-slide-in-right">

        {/* Header */}
        <div className="h-14 flex items-center px-5 gap-3 border-b border-tk-border bg-tk-bg2 flex-shrink-0">
          <button
            onClick={onClose}
            className="flex items-center gap-1.5 text-tk-text3 hover:text-tk-text transition-colors font-mono text-[11px] uppercase tracking-wide cursor-pointer"
          >
          ...
          {cliente && (
            <>
              ...
              <span className="font-mono text-[13px] font-semibold text-tk-text truncate">{cliente.nombre}</span>
              <div className="ml-1 flex-shrink-0"><BadgeEstado estado={cliente.estado} /></div>
            </>
          )}
          ...
          <div className="ml-auto flex items-center gap-2 flex-shrink-0">
            {editing ? (
              <>
                <button
                  onClick={() => { setEditing(false); setEditForm(cliente!); }}
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-tk-border2 text-tk-text2 font-mono text-[11px] uppercase tracking-wide cursor-pointer hover:border-tk-accent2 hover:text-tk-accent transition-colors"
              >
        ...
        {loading ? (
          ...
          <div className="flex flex-1 min-h-0 overflow-hidden">
            ...
            <div className="w-[280px] flex-shrink-0 border-r border-tk-border p-5 overflow-y-auto">
              ...
              {editing ? (
                <div className="space-y-3">
                  <div>
                    ...
                    <input className={inputCls} value={editForm.nombre ?? ''}
                      onChange={e => setEditForm(p => ({ ...p, nombre: e.target.value }))} />
                  </div>
                  <div>
                    <label className={labelCls}>Empresa</label>
                    <input className={inputCls} value={editForm.empresa ?? ''}
                      onChange={e => setEditForm(p => ({ ...p, empresa: e.target.value }))} />
                  </div>
                  <div>
                    <label className={labelCls}>Tipo de cliente</label>
                    <select className={inputCls} value={editForm.tipo_cliente ?? ''}
                      onChange={e => setEditForm(p => ({ ...p, tipo_cliente: e.target.value as TipoCliente | '' || null }))}>
                  ...
                  <div>
                    ...
                    <select className={inputCls} value={editForm.integrador ?? ''}
                      onChange={e => setEditForm(p => ({ ...p, integrador: e.target.value as Integrador | '' || null }))}>
                      <option value="">— Seleccionar —</option>
                  ...
                  <div>
                    ...
                    <select className={inputCls} value={editForm.estado ?? 'En Desarrollo'}
                      onChange={e => setEditForm(p => ({ ...p, estado: e.target.value as EstadoCliente }))}>
                      {ESTADOS.map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className={labelCls}>Origen de integración</label>
                    <select className={inputCls} value={editForm.origen_integracion ?? ''}
                      onChange={e => setEditForm(p => ({ ...p, origen_integracion: e.target.value as 'Directo' | 'Consolidador' | '' || null, consolidador: null }))}>
                      <option value="">— Seleccionar —</option>
                  ...
                  {editForm.origen_integracion === 'Consolidador' && (
                    <div>
                      ...
                      <input className={inputCls} value={editForm.consolidador ?? ''}
                        onChange={e => setEditForm(p => ({ ...p, consolidador: e.target.value }))} />
                    </div>
                  )}
                  <div>
                    <label className={labelCls}>Responsable</label>
                    <input className={inputCls} value={editForm.responsable ?? ''}
                ...
                <div className="space-y-3">
                  {[
                    ...
                    ...(cliente.origen_integracion === 'Consolidador'
                      ? [{ label: 'Consolidador', value: cliente.consolidador }]
                      : []),
                    { label: 'Responsable', value: cliente.responsable },
                  ].map(f => (
                    <div key={f.label} className="flex items-start gap-3">
                      <span className="font-mono text-[10px] text-tk-text3 uppercase tracking-[0.06em] w-24 flex-shrink-0 pt-0.5">
                        {f.label}
                      </span>
                      <span className="text-[13px] text-tk-text break-words min-w-0">
                        {f.value ?? <span className="text-tk-text3">—</span>}
                      </span>
                    </div>
                  ))}
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
              ...
              <div className="p-5 border-b border-tk-border flex-shrink-0">
                <p className="font-mono text-[10px] font-semibold text-tk-text3 uppercase tracking-[0.1em] mb-3">
                  Registrar actividad
                </p>
                <form onSubmit={handleAddActividad} className="flex flex-col gap-2.5">
                  ...
                  <div className="flex gap-2.5 items-end">
                    ...
                    <button
                      ...
                      className="px-4 py-2 rounded-lg text-white font-mono text-[11px] font-semibold uppercase tracking-wide cursor-pointer hover:opacity-90 transition-opacity disabled:opacity-40 flex-shrink-0"
                      style={{ background: '#CC0000' }}
                    >
                      {actSaving ? '…' : 'Registrar'}
                    </button>
              ...
              <div className="flex-1 overflow-y-auto p-5">
                ...
                {actividades.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 gap-3 text-tk-text3">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-8 h-8 opacity-30">
                      <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>
                    </svg>
                    <p className="font-mono text-[12px]">Sin actividades registradas aún</p>
                  </div>
                ) : (
                      ...
                      <div key={a.id} className="flex gap-3">
                        ...
                        <div className="flex-1 pb-4 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap mb-1.5">
                            ...
                            {a.responsable && (
                              <span className="text-[11px] text-tk-text2">{a.responsable}</span>
                            )}
                            <span className="text-[11px] text-tk-text3 ml-auto whitespace-nowrap">
                              {formatDate(a.created_at)}
                            </span>
                          </div>
                          <p className="text-[13px] text-tk-text leading-relaxed break-words whitespace-pre-wrap">
                            {a.descripcion}
                          </p>
                        </div>
                      </div>
