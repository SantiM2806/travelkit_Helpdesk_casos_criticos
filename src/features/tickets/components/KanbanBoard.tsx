...
const COLS = [
  ...
  { estado: 'En gestion del proveedor', norm: 'gestion',     colorCls: 'text-tk-orange', dotCls: 'bg-tk-orange', id: 'kCol-Gestion'    },
  { estado: 'Información cliente',      norm: 'infocliente', colorCls: 'text-tk-amber',  dotCls: 'bg-tk-amber',  id: 'kCol-InfoCliente'},
  { estado: 'Finalizado',               norm: 'finalizado',  colorCls: 'text-tk-green',  dotCls: 'bg-tk-green',  id: 'kCol-Finalizado' },
];
...
function KanbanCard({ ticket, onPointerDown }: KanbanCardProps) {
  ...
  const tipoBadge = badgeCat(ticket.tipo_solicitud || ticket.categoria || '');
  ...
  return (
    ...
    >
      ...
      {/* Cliente */}
      <div className="text-xs text-tk-text font-medium mb-[3px] whitespace-nowrap overflow-hidden text-ellipsis">
        {ticket.cliente || ticket.full_name || <span className="text-tk-text3">Sin cliente</span>}
      </div>

      {/* Agencia */}
      {(ticket.agencia || ticket.department) && (
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        {tipoBadge && <span className={tipoBadge.cls}>{tipoBadge.label}</span>}
      </div>
